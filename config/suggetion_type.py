...
class SuggestionType(resource.Resource):
	...
	def get(request):
		if 'id' in request.GET:
			suggestion_type = config_models.SuggestionType.objects.get(id=request.GET['id'])
		else:
	...
	def api_post(request):
		...
		id = data['id']
		del data['id']
		config_models.SuggestionType.objects.filter(owner=request.user, id=id).update(**data)
