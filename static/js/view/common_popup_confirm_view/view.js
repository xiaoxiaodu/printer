...
W.view.common.ConfirmView = W.view.common.DropBox.extend({
    ...
    getTemplate: function() {
        var template = '<dl class="itemDeleteView wui-confirmView">'
                +'<dd>'
                +  '<div class="xui-i-msg xa-msg"><span><%=msg%></span></div>'
                +  '<div class="mt5"><button type="button" class="xa-confirm btn xui-confirm btn-sm mr20">确定</button>'
                +  '<button type="button" class="xa-cancel xui-cancel btn btn-sm">取消</button></div>'
                +'</dd>'
                +'</dl>'
                +'<%=warning_msg%>';
