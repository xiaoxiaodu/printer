...
_.extend(W.component.Component.prototype, Backbone.Events, {
	...
	__getRenderContext: function() {
		...
		var items = context.type.split('.');
		context['minitype'] = items[1];

		return context;
	},

	render: function(options) {
		var context = this.__getRenderContext();
		var html = W.Render.render(context);
	...
	refresh: function($node, options) {
		if (W.Render) {
			...
			if ($componentContainer) {
				var context = this.__getRenderContext();
				var html = W.Render.refresh(context);
				$newNode = $(html);
				//删除旧节点
		...
		this.model.on('change', function(model) {
			...
			if (window.$M) { //window.$M将由mobile page通过parent.$M = $设置
				_.each(model.changed, function(value, name) {
					...
					if (handler) {
						//获得mobile page中的node
						var selector = '.xa-screen [data-cid="' + component.cid + '"]';
	 ...
	 *runtime: 管理运行时数据
	 */
	runtime: function(field, data) {
		if (data) {
			this.runtimeData[field] = data;
		} else {
			return this.runtimeData[field]
		}
	},
